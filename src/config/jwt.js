...
if (!jwtConfig.refreshtoken.secret) {
    throw new Error('JWT refresh token secret is not set in environment variables.');
}
if (!jwtConfig.refreshtoken.expiresIn) {
    throw new Error('JWT refresh token expiration time is not set in environment variables.');
}
if (!jwtConfig.refreshtoken.algorithm) {
    throw new Error('JWT refresh token algorithm is not set in environment variables.');
}
