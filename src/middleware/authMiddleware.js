...
const authenticateToken = async (req,res,next) => {
    try{
        ...
        if (!authHeader) {
            return res.status(404).json(BaseResponse.error('Authorization header is missing'));
        }
        ...
        if (!sessionValid) {
            return res.status(401).json(BaseResponse.error('Session is invalid or expired'));
        }
